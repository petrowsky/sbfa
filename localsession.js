...
function rawDataFullPageSession(data,tpageid,err,thisurl) {
		...
		if(startfunc == -1) {
			var msgObj ={ hostx: "oth", text: "UnrecognizedPage",url: thisurl };
...
function localSession(ev,inStr){
	...
	if(ev.text=='RestartSession') {
		...
		if(start==true) {
			...
			wwwTimer = setInterval(function () { wwwShowTimer();},1000);	
		}
  ...
  if(ev.text=='Visible') {	 
	if(wwwSession ==true) {
		...
		if((redTime-wwwElapsed)> yellowTime) {
			...
			postMsg({ hostx: 'www.', text: "HidePopups" }, id);
		}
		...
		if(wwwCloseLogoff==true){
			...
			if(wwwInitLogoff==true) {
				postMsg({ hostx: 'www.', text: "ShowLoggedOffPopup" }, id);
			}
		}					
	}
	...
	if(mSecRemain<=0) {
		...
		if(wwwCloseWarn==false){
			//if(wwwWarnShown==false) {
				var ev ={ hostx: 'www.', text: "ShowTimeoutWarning" ,endtime: ''}
    ...
    if(ev.data.hostx == host || ((ev.data.hostx=='oth') && (host != 'www.'))) {
	   ...
	   if(ev.data.text=='ShowLoggedOffPopup') {
		...
		if(isLocalLibraryCmd(ev.data.text )==false) {
			var msgObj ={ hostx: "oth", text: "Ack" ,msg:''};  
			if(host=="www.") {msgObj.hostx=host;}
...
document.addEventListener("visibilitychange",function () {
	if(document.visibilityState == "visible") {
		var msgObj ={ hostx: "oth", text: "Visible" };
		if(host=="www.") {msgObj.hostx=host;}
...
function sendPopupClosed() {
	var msgObj ={ hostx: "oth", text: "ClosePopup" };
	if(host=="www.") {msgObj.hostx=host;}
	sendTimerMsg(msgObj, "*");	
}
...
function resetLogoutTimer(thisurl) {
	...
	var msgObj ={ hostx: "oth", text: "RestartSession",sensitive: 'yes',url:'' };
	msgObj.url=thisurl;
...
function pokeSession(thisurl) {
		var msgObj ={ hostx: "oth", text: "RestartSession",sensitive: 'no',url:'' };
		msgObj.url=thisurl;
		if(host=="www.") {msgObj.hostx=host;}					
